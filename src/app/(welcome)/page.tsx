...
export default async function WelcomePage() {
  ...
  if (data.user) {
    const { data: pdata } = await server
      .from("users")
      .select(`username, fullname`)
      .eq("id", data.user.id)
      .single();
    profile.fullname = pdata?.fullname ?? null;
    profile.username = pdata?.username ?? null;
  }
  ...
  return (
    <div className="mx-auto flex min-h-screen flex-col justify-between text-primary">
      <nav
        className={
          "flex w-full items-center justify-between bg-background p-5"
        }>
        <Logo />
      ...
      <main className="w-100 flex flex-col items-center justify-center">
        <div className="mb-16 max-w-4xl text-center">
          ...
          <p className="mb-3 font-medium md:text-2xl">
            Station is the connected workspace where <br /> better, faster work
            happens.
          </p>
